...
public:
  ...
  //! \brief Generate ray passing through point (s, t) in the
  //!        viewport. s and t should be in [0, 1]. Note: inline
  //!        functions must be implemented inside the header file (and
  //!        of the .cc file)
...
/************MY extra functions for testing purposes only
  Vec3r GetLowerLeftCorner() const;
  Vec3r GetHorizontal() const;
  Vec3r GetVertical() const;
*/
protected:
  ...
  //! position of eye or the center of projection (cop).
...
//! \param[in] t vertical coordinate in [0, 1]
//! \return Created ray for point (s, t)
inline Ray Camera::GetRay(Real s, Real t) const {
  Ray ray;
  ray.SetOrigin(eye_);
  Vec3r P = lower_left_corner_ + s*horizontal_ + t*vertical_;
  ray.SetDirection(P-eye_);
  return ray;
}
