...
bool RayTracer::RayColor(const Ray &ray, Surface::Ptr scene, const std::vector<Light::Ptr> &lights, Vec3r &ray_color){
    ...
    //std::cout << "Ambient: " << phongMaterial->GetAmbient().transpose() << std::endl;
    //std::cout << "Diffuse: " << phongMaterial->GetDiffuse().transpose() << std::endl;
    //std::cout << "Specular: " << phongMaterial->GetSpecular().transpose() << std::endl;
    if (phongMaterial == nullptr) {
        ray_color = Vec3r{0, 0, 0};  // If it's not a PhongMaterial, no shading
        return false;
    }
    for (const auto& light : lights) {
        ray_color += light->Illuminate(hit_record, -ray.GetDirection());
    }
    return true;
}
...
RayTracer::Render(Surface::Ptr scene, const std::vector<Light::Ptr> &lights, Camera::Ptr camera)
{
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      ...
      rendered_image_.at<cv::Vec3d>(height - y -1, x) = cv::Vec3d{ray_color[0], ray_color[1], ray_color[2]};
